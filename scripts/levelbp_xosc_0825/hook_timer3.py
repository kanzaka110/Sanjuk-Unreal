...
for nid,n in nodes.items():
    if n["class"]=="K2Node_Literal":
        lab=n.get("title","").split("\n")[0]; pin=[p["name"] for p in n["pins"] if p["direction"]=="output"][0]; lit.setdefault(lab,(nid,pin))
print(lit)
MAP={"InAir12":"BP_EM_Hookshot_InAir12","Swing4":"BP_EM_Hookshot_Swing4","Swing5":"BP_EM_Hookshot_Swing5"}
end=[nid for nid,n in nodes.items() if n["class"]=="K2Node_VariableSet" and "HookPtOffset_Swing5" in json.dumps(n)][0]
Y=2200
...
slf=add(node_type="Self",position=[pos[0]+120,pos[1]+150])
