...
LOG["steps"].append("saved after create")
...
D("headO", "B", "0,0,170")
r = call("blueprint_query", "add_node",
         {"asset_path": BP, "graph_name": EG, "node_type": "CallFunction",
          "function_name": FN, "position": [700, 1200]})
EXT["callW"] = node_id_of(r)
call("blueprint_query", "set_node_property",
     {"asset_path": BP, "graph_name": EG, "node_id": EXT["callW"],
      "property_name": "FunctionReference", "value": '(MemberName="CalcWindAt",bSelfContext=True)'})
call("blueprint_query", "refresh_node", {"asset_path": BP, "graph_name": EG, "node_id": EXT["callW"]})
assert "Point" in node_pins(EG, EXT["callW"]), "callW Point 핀 미생성"
LOG["steps"].append("callW self-bound: %s" % EXT["callW"])
...
E(tm2["brCap"], "then", "callW")
E("callW", "then", tm2["loopT"], "Exec")
    ...
    for n in g2["nodes"]:
        for p in n.get("pins", []):
            if p.get("direction") != "input" or p.get("connected_to"):
                continue
            nm = p.get("name")
            if nm in ("Condition","InVec","Array","T","Location","TargetArray","Index","Point","NewItem","Item") or (nm in ("A","B") and not p.get("default_value")) or (nm == "self" and "VariableGet" in n.get("class","")):
                LOG["errors"].append({"unconnected": [G, n["id"], n.get("title"), nm]})
