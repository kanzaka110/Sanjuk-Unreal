# v15b — v15 ③부터 이어서 (2026-08-05)
#   판명: Turbulence/Size/Speed 는 베이스 SBWindComponent 선언 (self=base 타입) -> 신규 셋도 base 스폰
#   ①InitWind ②ApplyWind 는 v15에서 완료됨
import json
...
def spawn_baseset(graph: str, x: int, y: int, var: str) -> str:
    t3d = '(MemberParent=/Script/SB2.%s,MemberName="%s",bSelfContext=False)' % (BASE, var)
    nid = add(WBP, graph, "VariableSet", x, y, variable_name=var, target_class=BASE)
    ...
    if var not in pins:
        ...
        pins = node_pins(WBP, graph, nid)
    assert var in pins, "%s Set %s 핀 미생성" % (graph, var)
    return nid
...
vsTD = next(nid for nid in fn if nid.startswith("K2Node_VariableSet") and "Turbulence" in pmn(fn, nid))
...
vsTR = spawn_baseset(FN, 800, -300, "Turbulence")
vsTV = spawn_baseset(FN, 1050, -300, "Turbulence")
...
# ═══ ④ ApplyTurbSet: 3프로퍼티 -> Radial+Vortex (6셋) ═══
FN = "ApplyTurbSet"
fn = graph_nodes(WBP, FN)
entry_s = next(nid for nid in fn if "FunctionEntry" in nid)
vs3 = next(nid for nid in fn if nid.startswith("K2Node_VariableSet") and "TurbulenceSpeed" in pmn(fn, nid))
...
conns = []
x = 1250
for tvar, vg in (("TargetRadialComp", vgR), ("TargetVortexComp", vgV)):
    for var, param in (("Turbulence", "Turb"), ("TurbulenceSize", "Size"), ("TurbulenceSpeed", "Speed")):
        vs = spawn_baseset(FN, x, -300, var)
        conns += [
...
LOG["steps"].append("v15b 완료 — WBP 저장됨, ⚠ 맵 미저장 (레벨BP 변경 — 저장 필요)")
