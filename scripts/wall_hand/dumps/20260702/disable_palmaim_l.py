...
OUT = r"C:/Dev/Sanjuk-Unreal/scripts/wall_hand/dumps/20260702/palmaim_result.txt"
L=[]
try:
    ...
    # PalmAim_1.Weight 소스 링크 끊고 0 고정
    pa = nodes["PalmAim_1"]
    for p in pa.get_pins():
        if p.get_name()=="Weight":
            for s in p.get_linked_source_pins():
                ctrl.break_link(s.get_pin_path(), "PalmAim_1.Weight")
                L.append(f"break {s.get_pin_path()} -> PalmAim_1.Weight")
    ctrl.set_pin_default_value("PalmAim_1.Weight", "0.0")
    L.append("PalmAim_1.Weight=0")
