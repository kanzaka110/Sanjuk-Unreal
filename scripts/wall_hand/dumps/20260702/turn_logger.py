...
OUT = r"C:/Dev/Sanjuk-Unreal/scripts/wall_hand/dumps/20260702/turn_log.csv"
ROWS = ["time,turnAngle,speed2d,alpha,alphaTarget,bRight,actorYaw,smState"]
...
def _tick(dt):
    try:
        ...
        ROWS.append(f"{t:.3f},{float(ta):.1f},{float(sp):.1f},{float(al):.3f},{float(at):.3f},{int(bool(br))},{yaw:.1f},{int(sm.value) if hasattr(sm,'value') else sm}")
