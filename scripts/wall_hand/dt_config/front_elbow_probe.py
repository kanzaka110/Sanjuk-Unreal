# -*- coding: utf-8 -*-
"""정면 팔꿈치 폴 평면 검증 러너: 0/50도에서 양팔 팔축·팔꿈치 수직 오프셋."""
import io
import re
import sys
import time
...
for side,ua_,el_,hd_ in (('R','upperarm_r','lowerarm_r','hand_r'),('L','upperarm_l','lowerarm_l','hand_l')):
    ...
    print('RES %s rad=%.3f ax=(%.0f,%.0f,%.0f) perp=(%.1f,%.1f,%.1f)' % (side,rad,ax[0],ax[1],ax[2],perp[0],perp[1],perp[2]))
'''
...
PAT = re.compile(r"RES [RL] rad=[-0-9.]+ ax=\([-0-9., ]+\) perp=\([-0-9., ]+\)")
...
for deg, tag in ((0.0, "0도"), (50.0, "50도")):
    call("editor_query", {"action": "run_python",
                          "command": SETD.replace("VALUE", str(deg))}, timeout=20)
    time.sleep(1.5)
    sample(tag)
