...
├── shared_config.py               # 봇 공통 설정 (Codex CLI, 환경변수 검증)
├── .Codex/commands/              # 커스텀 슬래시 명령어 (10개, /hermes 포함)
├── .Codex/hooks/                 # Pre/PostToolUse 훅 (MCP 점검 3개) + Stop 훅(Hermes 자동공유, .Codex/settings.json)
├── .Codex/rules/                 # UE5 전용 룰 (도메인, MCP, 코딩, 매크로, 에이전트, 버전)
├── .Codex/agents/                # 프로젝트 전용 에이전트 (animbp/sim × inspector/tuner + ue-root-cause-reviewer)
...
- **SB2** — SHIFTUP SB2 메인 프로젝트 (커스텀 UE 5.7.4)
  ...
  - Monolith v0.20.2 (1232 액션 / 26 네임스페이스, 2026-06-19 measured) + UnrealClaude (포트 3000)
...
| [Sanjuk-Codex](https://github.com/kanzaka110/Sanjuk-Codex) | Codex 플러그인 (원본 리포) |
...
| 우선순위 | 도구 | 포트/방식 | 용도 |
|---------|------|----------|------|
| 1 (메인) | **Monolith** | HTTP `localhost:9316` | 에디터 제어 전체 (v0.20.2 — 1232 액션 / 26 네임스페이스, 2026-06-19 measured) |
...
Codex 실행 위치: `C:\dev\Sanjuk-Unreal` (루트)

- `.mcp.json` — MCP 서버 6개 등록 (monolith, unreal-mcp, unrealclaude-bridge, context7, maya, confluence)
- `.Codex/settings.local.json` — Codex 로컬 설정
- `.gitignore` — UE5 바이너리/임시 파일 제외

SB2 프로젝트는 `E:\Perforce\SB2\Workspace\Internal\SB2`에 위치.
MCP 서버들이 절대 경로/HTTP로 설정되어 있어 루트에서 Codex를 실행해도 UE 제어 가능.

## 리모트 세션 (모바일 접속)

GCP + 로컬 PC 두 세션 동시 운영. 모바일 Codex.ai/code에서 접속.
...
**동기화:** 세션 간 대화는 공유되지 않음. 중요한 컨텍스트는 이 AGENTS.md에 기록하고 git push/pull로 동기화.
...
- **간단 작업은 Sonnet** — `/model Codex-sonnet-4-6` 또는 명시. Opus는 복잡한 분석/그래프 작업만

## 세션 시작 시 필수 확인
- `~/Codex-sync/session-bridge.md` 를 읽어서 다른 세션의 최근 작업을 파악할 것
